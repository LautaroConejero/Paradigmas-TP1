...
void Guerreros::equipar_arma(unique_ptr<arma> a) {
    if (this->armas.first == nullptr) {
        this->armas.first = move(a);
    } else if (this->armas.second == nullptr) {
        this->armas.second = move(a);
    } else {
...
void Guerreros::eliminar_arma(int posicion) {
    if (posicion == 1) {
        this->armas.first = move(this->armas.second);
    }
    this->armas.second = nullptr;
    return;
}

pair<unique_ptr<arma>, unique_ptr<arma>>& Guerreros::Get_armas() {
    return this->armas;
}
