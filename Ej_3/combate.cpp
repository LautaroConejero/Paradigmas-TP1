...
int consola_combate(){
    ...
    if (decision == 1){
        ...
        cout << "2. Mago" << endl;
        cin >> tipo_personaje;
        while (tipo_personaje != 1 && tipo_personaje != 2){
            cout << "Opcion invalida. Elija nuevamente: " << endl;
            cin >> tipo_personaje;
        }
    } else {
        tipo_personaje = rand() % 2 + 1; // Genera un número aleatorio entre 1 y 2
    }
    shared_ptr<personaje> jugador = PersonajeFactory::Equipar_personaje(decision, cantidad_armas, tipo_personaje);
    shared_ptr<personaje> enemigo = PersonajeFactory::Equipar_personaje(2, cantidad_armas, tipo_personaje);
